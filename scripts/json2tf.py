...
def flatten_symbols(symbols,padded_length):
  flat_symbols = []
  lengths = []
  for syms in symbols:
    length = len(syms)
    padded_syms = syms + [0] * (padded_length - length)
    assert len(padded_syms) == padded_length
    flat_symbols.append(padded_syms)
    lengths.append(length)
  return {F.SYMBOLS.value: padded_syms,
          F.NUM_SYMBOLS_PER_POST.value: lengths}


def pad_symbols(syms,lens,padded_length,padding_char):
  padded_syms=list()
  first=0
  for length in lens:
    last=first+length
    tweet=syms[first:last]
    if len(tweet)<padded_length:
      padded_syms.extend(tweet+[padding_char]*(padded_length-len(tweet)))
    else:
      padded_syms.extend(tweet[:padded_length])
    first=last
  return {F.SYMBOLS.value: padded_syms}
